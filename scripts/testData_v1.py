...
def getData(images, labels):
    data = {int(i) : [] for i in set(labels) if int(i) > 9}
...
if __name__ == '__main__':
    f = [line.strip().split(',') for line in open('data/emnist/emnist-byclass-test.csv', 'r').readlines()]
    images, labels = [], []
    for i in f:
        label = i[0]
        image = np.float32([float(word) for word in i[1 : ]]).reshape((28, 28)).T
        images.append(image)
        labels.append(label)
    images = np.array(images)
    data = getData(images, labels)
    writeData('test-letters', data)
